...
const getAnchorAndDir = (origin, w, h) => {
  const outside = 0.2;
  switch (origin) {
    // Konfigurasi Arah Cahaya (Vector Direction)
    case "top-left":
      return { anchor: [0, -outside * h], dir: [0.5, 0.5] };
    case "top-right":
      return { anchor: [w, -outside * h], dir: [-0.5, 0.5] }; // X=-0.5 (Kiri), Y=0.5 (Bawah)
    case "bottom-left":
...
// --- KONFIGURASI DI SINI ---
const rays = new LightRays("[data-rays-wrap] [data-canvas]", {
  raysOrigin: "top-right", // <--- SET KE 'top-right'
  raysColor: "#ffdd00", // <--- SET WARNA DI SINI (Contoh: Emas)
  raysSpeed: 0.5,
  mouseInfluence: 0.1,
});
