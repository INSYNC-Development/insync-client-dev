...
const AsiamedApp = {
  ...
  helpers: {
    validateLibraries() {
      ...
      if (missing.length > 0) {
        console.warn(
          `[AsiamedApp] Missing libraries: ${missing.join(
            ", "
    ...
    // Runs cb() once the page is actually visible to the user.
    //
    // The Webflow head embed holds `html:not(.wf-design-mode) body` at
    // opacity 0 and a Webflow IX2 page-load interaction sets the inline
    // opacity:1 that reveals it. That happens well after
    // document.fonts.ready (~1.9s vs ~1.3s on staging), so anything that
    // animates straight out of init() burns most of its runtime behind a
    // blank screen. Anything load-triggered and once-only should gate on
    // this instead of guessing a delay.
    isPageRevealed() {
      return (
        parseFloat(window.getComputedStyle(document.body).opacity || "1") > 0.99
      );
    },
    whenPageRevealed(cb) {
      const isRevealed = () => AsiamedApp.helpers.isPageRevealed();
      ...
      // IX2 reveals the body by writing an inline style on <body>.
      ...
      // Safety net: if the interaction is ever removed or errors out, the
      // body would stay at opacity 0 and cb() must still run — otherwise
      // whatever it collapsed in CSS would sit invisible for good.
      const bail = setTimeout(run, 4000);
    },
    padZero(num) {
  ...
  components: {
    initTechnicSwiper() {
      ...
      new Swiper(".swiper.is-technic", {
        ...
        slideClass: "swiper-slides.is-technic",
        navigation: {
    ...
    initHandleSwiper() {
      ...
      if (originalSlides.length > 0) {
        while (wrapper.querySelectorAll(".swiper-slides").length < MIN_SLIDES) {
  ...
  animations: {
    ...
    initAkupunturSectionAnimation() {
      // Opacity awal judul — MOBILE SAJA (<=767px).
      //
      // Keluhan client-nya khusus smartphone ("briefly confusing on smartphone
      // since nothing is visible at first"), di layar kecil section-nya terasa
      // kosong sebelum di-scroll. Di tablet ke atas judulnya tetap mulai dari 0
      // seperti desain awal. Dicek sekali saat init, sama seperti guard
      // breakpoint lain di file ini (initHighlightMarkerTextReveal,
      // initBeginingYearFlip) — bukan lewat gsap.matchMedia.
      const AKUPUNTUR_TITLE_START_OPACITY = window.matchMedia(
        `(max-width: ${AsiamedApp.config.breakpoints.mobile}px)`
      ).matches
        ? 0.05
        : 0;
      ...
      // Di mobile judulnya mulai abu-abu (sudah terbaca sebelum di-scroll),
      // di tablet ke atas mulai dari 0 — lihat konstanta di atas.
      gsap.set(splitText.words, { opacity: AKUPUNTUR_TITLE_START_OPACITY });
    ...
    initKinesiologieSectionAnimation() {
      // Opacity awal judul — MOBILE SAJA (<=767px), sama alasannya dengan
      // section Akupunktur di atas: abu-abu hanya untuk smartphone, tablet ke
      // atas tetap mulai dari 0.
      const KINESIOLOGIE_TITLE_START_OPACITY = window.matchMedia(
        `(max-width: ${AsiamedApp.config.breakpoints.mobile}px)`
      ).matches
        ? 0.05
        : 0;
      ...
      // Di mobile judulnya mulai abu-abu, di tablet ke atas mulai dari 0 —
      // lihat konstanta di atas.
      gsap.set(words, { opacity: KINESIOLOGIE_TITLE_START_OPACITY });
    ...
    initSectionLineReveal() {
      ...
      // A wrap above the fold satisfies triggerStart the moment its
      // ScrollTrigger is created, so nothing may be built until the page is
      // actually on screen. Measured on staging 2026-08-18: init() runs at
      // document.fonts.ready but the Webflow IX2 page-load interaction only
      // reveals <body> at ~1.9s, by which point the hero grid was already
      // ~69% drawn — the client saw a sliver of movement and reported the
      // lines as "still not animated". The old fixed 0.6s head start could
      // never cover a reveal that late, and a bigger number is still a guess.
      // Re-entry settles synchronously once the body is visible.
      if (!AsiamedApp.helpers.isPageRevealed()) {
        AsiamedApp.helpers.whenPageRevealed(() => this.initSectionLineReveal());
        return;
      }

      // Slowed down 2026-08-18 (0.5s / 0.08s / 0.15s before). At the old speed
      // the grid was drawn almost as soon as it started — it read as a state
      // change rather than an animation, which is the other half of the client
      // note about the lines "not being animated". A 13-line wrap now sweeps
      // over ~2.6s total (0.14 × 12 + 0.9) instead of ~1.5s, so the draw is
      // legible as motion. Per-instance overrides still win: see the
      // data-section-line-stagger / data-section-line-delay reads below.
      const config = {
        duration: 1.2,
        stagger: 0.4,
        topLead: 0.25,
        ease: "power3.out",
        triggerStart: "top 85%",
        // Only a small beat after the page reveal (see the gate above), not
        // a guess at when the reveal happens.
        initialDelay: 0.15,
      };
    ...
    initKongressClick() {
      const headers = document.querySelectorAll(".kongress_card_wrap");
      if (!headers.length) return;

      // Slowed and softened 2026-08-18 on client feedback: *"the transition
      // should be a bit more subtle"*. power3 spends longer in its slow tail
      // than power2, so the card settles rather than snapping into place.
      // (Was: 0.6s power2.out open / 0.6s power2.inOut close.)
      const config = {
        openDuration: 1,
        openEase: "power3.out",
        closeDuration: 0.8,
        closeEase: "power3.inOut",
        textOpenDuration: 0.6,
        textOpenStagger: 0.06,
        textCloseDuration: 0.4,
        textCloseStagger: 0.03,
      };

      // How far the card rests below the hero — read from the stylesheet at
      // CLICK time, never cached at init.
      //
      // Webflow sets it as a percentage that differs per breakpoint (-62.5%
      // desktop / -58% <=991px / -52.5% <=767px) and getComputedStyle resolves
      // that to px. The old code measured once during init() and reused the
      // number forever, so the value went stale the moment the viewport
      // crossed a breakpoint. Verified on staging 2026-08-18: load at 1440px,
      // resize to 500px, open and close — the card landed on the desktop
      // -375px instead of the mobile -315px, and because that px value is
      // written inline it then outranked the responsive rule permanently
      // (the peek shrank from the intended 88px to 28px).
      //
      // Reading it back means clearing the inline value first; that happens
      // inside one synchronous block, so nothing is painted in between. The
      // caller must remove .is-open BEFORE calling this, or the stylesheet
      // answers with the open position (0%) instead of the resting one.
      const restingBottom = (el) => {
        const inline = el.style.bottom;
        el.style.bottom = "";
        const resting = window.getComputedStyle(el).bottom;
      ...
      headers.forEach((header) => {
        ...
        header.addEventListener("click", () => {
          ...
          if (isOpen) {
            // Remove the class first — restingBottom() reads the stylesheet,
            // and .is-open would answer with the open position.
            currentCardWrap.classList.remove("is-open");

            gsap.to(currentCardWrap, {
              bottom: restingBottom(currentCardWrap),
            ...
            if (textElements.length > 0) {
              gsap.to(textElements, {
                opacity: 0,
                duration: config.textCloseDuration,
                ease: config.closeEase,
                stagger: config.textCloseStagger,
              });
            }
          } else {
            // Pin the current position inline BEFORE adding the class.
            // .is-open carries `bottom: 0%` in the Webflow stylesheet, and on
            // the first click after a page load there is no inline value for
            // it to lose to — so the class moved the card to the open
            // position before GSAP had even read its start value, and it
            // popped instead of sliding. Verified on staging 2026-08-18:
            // fresh load, first click, -375px -> 0 in a single frame with no
            // tween at all. (Later clicks looked fine only because the close
            // tween had left an inline value behind, which outranks a class.)
            // This is the "transition" the client asked to make subtler —
            // there wasn't one on first open.
            gsap.set(currentCardWrap, {
              bottom: window.getComputedStyle(currentCardWrap).bottom,
            });
            ...
            if (textElements.length > 0) {
              gsap.to(textElements, {
                opacity: 1,
                duration: config.textOpenDuration,
                ease: config.openEase,
                stagger: config.textOpenStagger,
              });
            }
          }
        });
  ...
  init() {
    ...
    safeInit("Section Hash Deep Link", () => this.animations.initHashScroll());
