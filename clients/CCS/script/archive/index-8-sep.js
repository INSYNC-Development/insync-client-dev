...
function initLogoRevealLoader(onCompleteCallback) {
  ...
  const loaderHasPlayed = sessionStorage.getItem("loaderPlayed");
...
function aboutSequenceAndReveal() {
  ...
  const config = {
    baseUrl:
      "https://cdn.jsdelivr.net/gh/INSYNC-Development/insync-client-dev@main/clients/CCS/optimized-assets/sequences/machine-reveal-v2/1440/",
    frameCount: 152,
    padding: 4,
    suffix: "",
    extension: ".webp",
  ...
  // Kalau frame target belum selesai diunduh, pakai frame terdekat yang sudah
  // ada supaya canvas tidak pernah kosong saat scroll cepat.
  function resolveFrame(index) {
    if (images[index] && images[index].complete) return index;
    // Cari mundur maksimal 12 langkah - jangan menyapu 140 frame tiap tick.
  ...
  function render() {
    ...
    // onUpdate dipanggil ~60x/detik, tapi "snap: frame" bikin nilainya sering
    // sama. Tanpa penjaga ini sekitar sepertiga draw call terbuang untuk
    // menggambar ulang frame yang identik.
  ...
  function sizeCanvas() {
    ...
    // Naikkan resolusi HANYA sampai batas yang bisa diisi sumbernya.
    // Sumber 1440px di kotak 1081px CSS berarti dpr efektif 1.33, bukan 2.
    // Memaksa 2 menghasilkan buffer 3.84MP - 4x kerja raster, tanpa
    // tambahan detail sedikit pun, karena filenya memang cuma 1440px.
    ...
    // Set sekali di sini, bukan tiap frame. Mengubah canvas.width mereset
    // state context, jadi harus dipasang ulang setelah resize.
  ...
  function preloadImages() {
    for (let i = config.startFrame + 1; i < config.frameCount; i++) {
      ...
      images[i] = img;
      // Decode di luar jalur render. Tanpa ini, drawImage pertama untuk tiap
      // frame memicu decode WebP sinkron di main thread - inilah yang terasa
      // sebagai tersendat saat scroll cepat melewati frame yang baru.
...
document.addEventListener("DOMContentLoaded", () => {
  document.fonts.ready.then(() => {
    initLogoRevealLoader(initMainAnimations);
  });
});
