...
function heroHoverAnim() {
    const container = document.querySelector(".hero_wrap");
  ...
  const hoverVisualAnim = () => {
    ...
    hoverTl
      ...
      .to(
        card1,
        {
          bottom: "30%", // from 25% → nudge slightly (was 45% → 27%)
          right: "4%", // from 10% → 2%
          // opacity: 0.65,
        },
        0
      ...
      .to(
        card3,
        {
          bottom: "25%", // stays roughly the same or fine-tune
          right: "0%", // from 0% (default) stays
        },
        0
      );
  
    // Play on hover, reverse on leave
    card3.addEventListener("mouseenter", () => hoverTl.play());
    card3.addEventListener("mouseleave", () => hoverTl.reverse());
  };
  ...
  function initIntegrationHover() {
    ...
    // Configuration: "Idle/Away" positions in Percent
    const config = {
      outlook: { x: -50, y: 130 }, // Top-Right pull
      finance: { x: 20, y: 450 }, // Bottom-Left pull
      meeting: { x: 0, y: 55 }, // Bottom pull
    };
  
    // Helper: Start the idle Yoyo animation from the offset position
    function runIdleAnimation() {
      ...
      // 2. Finance Loop (delayed slightly)
      gsap.fromTo(
        finance,
    ...
    // A. ScrollTrigger - Start animation when in view
    ScrollTrigger.create({
      trigger: triggerSection,
      start: "top 70%", // Triggers when top of section hits 70% of viewport
      onEnter: () => runIdleAnimation(),
      once: true, // Runs once so it doesn't reset aggressively on scroll up
    });
  
    // B. Hover Events
    wrap.addEventListener("mouseenter", () => {
      // Force move to center (0,0) - Overwrites the yoyo loops
      gsap.to(items, {
        xPercent: 0,
        yPercent: 0,
        duration: 0.6,
    ...
    wrap.addEventListener("mouseleave", () => {
      // Return to offset positions, then restart the yoyo loop
      const tl = gsap.timeline({ onComplete: runIdleAnimation });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // ── Selectors (updated to new class names) ──
    ...
    btnWraps.forEach((btnWrap) => {
      ...
      const glowPrimary = btnWrap.querySelector('[data-glow="primary"]');
      const glowSecondary = btnWrap.querySelector('[data-glow="secondary"]');
      const outerGlows = btnWrap.querySelectorAll("[data-outer-glow]");
  
      // ── Config ──
      const GRADIENT_WIDTH = 204;
      const REST_X = 114;
  
      const CENTER_DEAD_ZONE = 0.25;
  
      // ── Resting state: glow visible on the right ──
      const REST_ANGLE = 90;
      ...
      // ── Set outer glows to resting state (right-side glow visible) ──
      ...
      // ── Inner gradient cursor-follow ──
      function moveGradient(e) {
        const rect = cta.getBoundingClientRect();
        const mouseX = e.clientX - rect.left;
        const targetX = mouseX - GRADIENT_WIDTH / 1.5;
