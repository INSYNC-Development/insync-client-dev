...
// -----------------------------------------
// PAGE SCOPE (teardown on Barba navigation)
// -----------------------------------------
//
...
// installs its own resize/font observers), Swiper instances and raw window
// listeners all survive the swap. Their cleanup callbacks only fire on a media
// query change, so after N navigations the page carries N stacked copies of
// everything — every scroll/resize then re-runs N× of work against detached
// DOM, which is what freezes the page.
//
...
function initialFunction(container) {
  ...
  [
    ...
    initModals,
    initFilterProject,
  ].forEach((task) => runPageTask(task, container));

  // The functions below build/measure a lot of DOM (SplitText, roller
  // markup, pinned ScrollTriggers, Swiper instances). Running them all in
  // one synchronous task after Barba swaps in a fully-built container can
  // block the main thread long enough to freeze the page and delay input
  // (scroll only "catches up" once the task finishes). Spreading them
  // across animation frames lets the browser paint and process input
  // between each one.
  runHeavyPageTasks(container, [
...
function whySlider(container) {
  ...
  function duplicateImageSlides() {
    ...
    // With an empty CMS list `i % 0` is NaN, so itemSlides[NaN] is undefined
    // and .cloneNode() throws.
  ...
  let resizeTimeout;
  addPageListener(window, "resize", () => {
    clearTimeout(resizeTimeout);
    resizeTimeout = setTimeout(() => {
      buildSplitText();
    }, 250);
  });
  onPageDestroy(() => clearTimeout(resizeTimeout));
...
function initNumberOdometer(container) {
  ...
  onPageDestroy(() => clearTimeout(resizeTimer));
...
function initLineRevealTestimonials(container) {
  ...
  wraps.forEach((wrap) => {
    ...
    slides.forEach((slide, slideIndex) => {
      ...
      trackSplits(slide.splitInstances);
    });
    ...
    addPageListener(window, "keydown", onKeyDown);

    // startAutoplay() reschedules itself forever; without this the timer keeps
    // driving a detached slider after the page is swapped out.
    onPageDestroy(() => {
      if (autoplayCall) autoplayCall.kill();
      autoplayCall = null;
    });
...
function initStoryAnimation(container) {
  ...
  wraps.forEach((wrap) => {
    ...
    const currentEls = wrap.querySelectorAll("[data-current]");
    const totalEls = wrap.querySelectorAll("[data-total]");
    ...
    function startAutoplay() {
      ...
      autoplayTimer = gsap.delayedCall(autoplayDuration / 1000, () => {
    ...
    function pauseAutoplay() {
      if (autoplayTimer) autoplayTimer.pause();
    }

    function resumeAutoplay() {
      if (autoplayTimer) autoplayTimer.resume();
    }
...
function setupTextLinesReveal(container) {
  ...
  const textEls = gsap.utils.toArray(
    scope.querySelectorAll("[data-heading-reveal] strong")
  );
      ...
      serviceItems.forEach((item, index) => {
        ...
        item.addEventListener("click", () => {
          ...
          const targetScrollPosition =
            scrollStart + totalScrollDistance * targetProgress;
...
function initFilterProject(container) {
  ...
  const items = section.querySelectorAll('[data-content="item"]');
  ...
  items.forEach((item) => {
    ...
    if (categoryEl && categoryEl.textContent.trim() !== "") {
      categories.add(categoryEl.textContent.trim());
    }
  });
  ...
  const allFilterButtons = section.querySelectorAll('[data-filter="item"]');
  ...
  allFilterButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      items.forEach((item) => {
        const itemCatEl = item.querySelector('[data-filter="category"]');
        const itemCategory = itemCatEl ? itemCatEl.textContent.trim() : "";
...
function loadMoreBlogs(container) {
  ...
  const items = section.querySelectorAll('[data-blog="item"]');
...
function formInputValidation(container) {
  ...
  async function validateEmailDomain(domain, errorElement) {
    try {
    ...
    emailInputs.forEach((emailInput) => {
      ...
      emailInput.addEventListener("blur", function () {
        ...
        if (emailParts.length === 2 && emailParts[1].length > 0) {
          ...
          validateEmailDomain(domain, errorElement);
        } else {
    ...
    telInputs.forEach((telInput) => {
      ...
      telInput.addEventListener("input", function () {
        ...
        if (this.value.length > 0 && !/^\+?\d+$/.test(this.value)) {
          showError("Only numbers and a leading + are allowed.", errorElement);
        } else {
          hideError(errorElement);
        }
      });
...
function initModals(container) {
  ...
  container.querySelectorAll(".modal_dialog").forEach((modal) => {
...
function initWorkStack(container) {
  const cards = gsap.utils.toArray(
    container.querySelectorAll(".work-m_item_wrap")
  );

  if (!cards.length) return;

  cards.forEach((card, i) => {
    // card.style.setProperty("--space", `${i * 16}px`);
...
function initStepAnimation(container) {
  ...
  mm.add("(min-width: 992px)", () => {
    ...
    const textItems = stepWrap.querySelectorAll('[data-content="item"]');
    const visualItems = stepWrap.querySelectorAll('[data-visual="item"]');
    const descItems = stepWrap.querySelectorAll('[data-content="desc"]');
    ...
    function goToStep(index) {
      ...
      if (prevIndex >= 0) {
        gsap.to(descItems[prevIndex], {
          ...
          duration: 0.4,
          overwrite: "auto",
          onComplete: () => gsap.set(descItems[prevIndex], { display: "none" }),
        });
      }
