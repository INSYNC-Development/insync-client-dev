...
function initialFunction(container) {
  ...
  copyrightYear(container);
  loadMoreBlogs(container);
  initModals(container);
  initWorkStack(container);
  initFilterProject(container);
  initStepAnimation(container);
        ...
        .to(
          split.chars,
          {
            keyframes: [
              ...
              {
                color:
                  "color-mix(in srgb, var(--_theme---text) 40%, transparent)",
                duration: 0.35,
              },
            ],
            stagger: { each: 0.01 },
...
function initFilterProject(container) {
  ...
  const allFilterButtons = document.querySelectorAll('[data-filter="item"]');
...
function initWorkStack(container) {
  ...
  cards.forEach((card, i) => {
    console.log(`Index: ${i}, Elemen:`, card);
...
function initStepAnimation(container) {
  ...
  mm.add("(min-width: 992px)", () => {
    ...
    descItems.forEach((desc, index) => {
      const originalStyle = desc.getAttribute("style") || "";
      gsap.set(desc, { display: "block", autoAlpha: 1 });

      const parentSplit = new SplitText(desc, {
        type: "lines",
      ...
      descSplits.push({ parent: parentSplit, child: childSplit });

      desc.setAttribute("style", originalStyle);

      if (index === 0) {
        gsap.set(desc, { display: "block", autoAlpha: 1 });
        gsap.set(childSplit.lines, { yPercent: 0 });
      } else {
        gsap.set(desc, { display: "block", autoAlpha: 0 });
        gsap.set(childSplit.lines, { yPercent: 110 });
      }
    });
    ...
    let st = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        if (activeIndex !== currentIndex) {
          ...
          if (currentIndex >= 0 && descSplits[currentIndex]) {
            gsap.to(descSplits[currentIndex].child.lines, {
              yPercent: -110,
              duration: 0.6,
              ease: "power4.inOut",
              stagger: { amount: 0.25 },
          ...
          if (descSplits[activeIndex]) {
            gsap.set(descItems[activeIndex], { autoAlpha: 1 });
            gsap.fromTo(
              descSplits[activeIndex].child.lines,
              { yPercent: 110 },
              {
                yPercent: 0,
                duration: 0.7,
