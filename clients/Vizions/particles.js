...
gl_PointSize = (uBaseSize * (1.0 + uSizeRandomness * (random.x - 0.5))) / (length(mvPos.xyz) * 0.5 + 5.0);
}
...
void main() {
  ...
  if(uAlphaParticles < 0.5) {
    ...
    // KUNCI: Kalikan nilai RGB dengan alpha (circle)
    gl_FragColor = vec4(vec3(1.0) * circle, circle);
  }
}
`;
  ...
  update(t) {
    ...
    if (this.options.moveParticlesOnHover) {
      this.particles.position.x =
        -this.mouseX * this.options.particleHoverFactor;
      this.particles.position.y =
        -this.mouseY * this.options.particleHoverFactor;
    } else {
...
if (window.innerWidth > 768) {
  const myParticles = new ParticleSystem("#particles-container", {
    particleCount: 150,
    moveParticlesOnHover: false,
    particleColors: ["#fff"],
    speed: 0.05,
