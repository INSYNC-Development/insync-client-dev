...
function initGlossaryContent() {
  ...
  resetText.className = "glossary_text u-text-style-h4";
  resetText.textContent = "ALL";
  resetText.setAttribute("data-button", "reset");
  resetText.setAttribute("data-state", "active");
  targetCategory.appendChild(resetText);
...
function initGlossaryLetterFilter() {
  ...
  const categoryButtons = group.querySelectorAll(
    ".glossary_text[data-category]"
  );
  const categoryItems = group.querySelectorAll(".glossary_content_item");
  const resetScope = group.querySelector('[data-button="reset"]')
    ? group
    : document;
  const resetButtons = [
    ...resetScope.querySelectorAll('[data-button="reset"]'),
  ];
  const resetChip = group.querySelector('.glossary_list [data-button="reset"]');

  const searchInput =
    group.querySelector('[data-filter="search"]') ||
    document.querySelector('input[type="text"], input[type="search"]');

  let activeLetter = null;

  const syncResetState = () => {
    resetChip?.setAttribute("data-state", activeLetter ? "inactive" : "active");
  };
...
/* Location is CMS free text ("Essen, Germany"), so filtering on the raw value
   produced 88 checkboxes for 313 entries — including near-duplicates like
   "Munich, Germany" / "München, Germany". Rolling each value up to its country
   collapses that to 18 and dissolves the spelling variants. */
const LOCATION_ALIASES = {
  // Not a country. UBS is listed as "Zürich, Switzerland / DACH".
  dach: ["germany", "austria", "switzerland"],
};

function toCountries(value) {
  ...
  parts.forEach((part) => {
    ...
    if (comma !== -1) {
      seenQualified = true;
      found.push(part.slice(comma + 1));

      return;
    }

    // A "/" part with no comma is ambiguous, and position decides it: before
    // the first "City, Country" part it is a second city ("Königstein/
    // Frankfurt, Germany"), after it a second country ("Frankfurt, Germany /
    // Austria / Switzerland"). A lone part is already a bare country name.
    if (seenQualified || parts.length === 1) found.push(part);
  });

  // Nothing usable — fall back to the last segment so the entry stays
  // filterable and the bad data surfaces instead of silently vanishing.
  if (!found.length) found.push(parts[parts.length - 1]);
...
/* Keys whose raw CMS values get rolled up before they reach the filter UI.
   Both the checkbox list and the matching read from here, so they cannot drift
   apart. Blog has no "location" key, so it is untouched. */
const FILTER_DERIVE = {
  location: toCountries,
};

function applyDerive(key, values) {
  const derive = FILTER_DERIVE[key];

  if (!derive) return values;

  return [...new Set(values.flatMap((value) => derive(value)))];
}
...
function getItemDetails(item) {
  ...
  item.querySelectorAll("[data-type]").forEach((typeGroup) => {
    ...
    if (listItems.length) {
      details[key] = applyDerive(
        key,
        Array.from(listItems).map((el) =>
    ...
    if (singleValue) {
      details[key] = applyDerive(key, [normalizeText(singleValue)]);
    }
  });
...
/* ================================
  Filter UI
================================ */
function isFilterCollapsible(wrap) {
  // Mobile has always collapsed. Desktop opts in per sidebar so the change can
  // ship on one page at a time — add data-filter-collapse="true" in Webflow.
  return (
    window.matchMedia("(max-width: 991px)").matches ||
    wrap?.getAttribute("data-filter-collapse") === "true"
  );
}

function createCheckboxFilter(wrap, key, values) {
  const isTabletDown = window.matchMedia("(max-width: 991px)").matches;
  const isCollapsible = isFilterCollapsible(wrap);

  const filterItem = document.createElement(isCollapsible ? "details" : "div");
  ...
  if (isCollapsible) {
    if (isTabletDown) {
      // Narrow screens keep the exclusive accordion they already had.
      filterItem.setAttribute("name", "filter");
    } else {
      // On desktop the groups open independently and start open, so the page
      // looks unchanged until the user chooses to collapse something. Closing
      // a group hands its height to the ones still open (registry-filter.css).
      filterItem.setAttribute("open", "");
    }
    ...
    filterItem.innerHTML = `
      ...
      <div class="filter_content_detail" data-lenis-prevent>
        <div class="filter_list"></div>
      </div>
    `;

    // "toggle" does not bubble, so it has to be bound per group.
    filterItem.addEventListener("toggle", () => layoutFilterScroll(wrap));
  } else {
    filterItem.innerHTML = `
      <div class="filter_content_text u-text-style-main u-color-faded u-text-transform-capitalize">
        ${key}
...
// Filters are built once for the width at load, so crossing the breakpoint
// afterwards left them in the wrong accordion mode. Swapping the exclusivity
// flag here is enough — the markup itself is identical on both sides.
function updateFilterDetailsMode() {
  ...
  document.querySelectorAll(".filter_content_item").forEach((item) => {
    ...
    if (isTabletDown) {
      ...
      // Only on the way in from mobile, so resizing across desktop widths
      // does not reopen a group the user deliberately closed.
...
/* Hands the sidebar's height budget to the groups that are open.
   Flexbox alone shrinks every group in proportion to its size, which starves a
   3-option group just as hard as an 18-option one. This fills smallest-first
   instead: a group that already fits is never capped, and only the long ones
   get a scroll area out of what is left. */
const FILTER_SCROLL_MIN = 96; // px — a capped group still shows ~3 rows

function layoutFilterScroll(wrap) {
  if (!wrap || wrap.getAttribute("data-filter-collapse") !== "true") return;

  const groups = [...wrap.querySelectorAll("details.filter_content_item")];
  if (!groups.length) return;

  const details = groups.map((group) =>
    group.querySelector(".filter_content_detail")
  );

  // Always start from the uncapped state so measurements are the natural ones.
  details.forEach((detail) => {
    if (detail) detail.style.maxHeight = "";
  });

  // Below 992px the sidebar is static and the page scrolls it — nothing to cap.
  if (window.matchMedia("(max-width: 991px)").matches) return;

  const budget = parseFloat(getComputedStyle(wrap).maxHeight);
  if (!budget) return;

  const styles = getComputedStyle(wrap);
  const gap = parseFloat(styles.rowGap) || 0;

  let remaining = budget - gap * Math.max(0, groups.length - 1);

  groups.forEach((group) => {
    if (group.open) {
      remaining -=
        group.querySelector(".filter_content_head")?.offsetHeight || 0;
    } else {
      remaining -= group.offsetHeight;
    }
  });
  ...
  const open = groups
    ...
    .map((group) => ({
      detail: group.querySelector(".filter_content_detail"),
      natural: group.querySelector(".filter_content_detail")?.scrollHeight || 0,
    }))
    .filter((entry) => entry.detail)
    .sort((a, b) => a.natural - b.natural);

  let share = open.length;

  open.forEach((entry) => {
    const allowance = remaining / share;

    if (entry.natural <= allowance) {
      // Fits already — leave it uncapped and pass the slack to the rest.
      remaining -= entry.natural;
    } else {
      const capped = Math.max(allowance, FILTER_SCROLL_MIN);

      entry.detail.style.maxHeight = `${Math.floor(capped)}px`;
      remaining -= capped;
    }
...
function layoutAllFilterScroll() {
  document
    .querySelectorAll('[data-filter="wrap"][data-filter-collapse="true"]')
    .forEach((wrap) => layoutFilterScroll(wrap));
}
...
function renderAutoFilters(group, data) {
  ...
  Object.entries(filters).forEach(([key, values]) => {
    ...
    // Derived options are machine-generated, so their order is just whichever
    // entry happened to come first in the CMS. Sort those; leave hand-curated
    // keys in the order the editor arranged them.
    if (FILTER_DERIVE[key]) options.sort((a, b) => a.localeCompare(b));

    createCheckboxFilter(wrap, key, options);
  });
}
...
function initButtonAnimation() {
  ...
  const buttons = gsap.utils
    .toArray(BTN_SELECTOR)
    .filter((button) => !button.closest(".nav_mobile_wrap.w-nav"));

  if (!buttons.length) return;

  const setup = (button) => {
    if (button._split) return true;

    const textEl = button.querySelector(TEXT_SELECTOR);
    if (!textEl) return false;

    const split = SplitText.create(textEl, {
      type: "chars", // "lines" dibuang -> jauh lebih murah
      charsClass: "char",
      mask: "chars",
    });
    ...
    split.chars.forEach((char) => {
      const value = char.textContent;
      if (value.trim() !== "") char.setAttribute("data-char", value);
    });

    button._split = split;

    // timeline dibuat sekali, tidak bikin tween baru tiap hover
    button._tl = gsap
      .timeline({ paused: true })
      .fromTo(
        split.chars,
        { yPercent: 0 },
        { yPercent: -100, duration: 0.6, ease: "power2.out", stagger: 0.025 }
      )
      .set(split.chars, { yPercent: 0 });
...
// jalankan setelah font siap & main thread longgar
document.fonts.ready.then(() => {
  requestIdleCallback
    ? requestIdleCallback(initButtonAnimation)
    : setTimeout(initButtonAnimation, 0);
});
...
function initHeroTab() {
  const getButtons = () => [...document.querySelectorAll("[data-tab-title]")];
  const getPanels = () => [...document.querySelectorAll("[data-tab-image]")];

  const buttons = getButtons();
  const panels = getPanels();

  if (!buttons.length || !panels.length) return;
  if (document._heroTabInitialized) return;
  document._heroTabInitialized = true;

  const keys = new Set(panels.map((p) => p.getAttribute("data-tab-image")));
  ...
  const activate = (key) => {
    ...
    getPanels().forEach((panel) => {
      panel.style.display =
        panel.getAttribute("data-tab-image") === key ? "block" : "none";
    });

    getButtons().forEach((button) => {
      button.setAttribute(
        "data-tab-state",
        button.getAttribute("data-tab-title") === key ? "active" : "inactive"
      );
    });
  };

  // .main-hero_list is a draggable GSAP marquee. Its own drag guard only covers
  // <a>, and these tabs are <div>/<button>, so a drag released over a tab would
  // otherwise switch it. Treat a press as a click only if it barely moved.
  let downX = 0;
  let downY = 0;
  ...
  document.addEventListener("click", (e) => {
    const button = e.target.closest?.("[data-tab-title]");
    if (!button) return;

    // e.detail === 0 means keyboard or programmatic — no pointer travel to check.
    ...
    activate(button.getAttribute("data-tab-title"));
  });

  const firstMatch = buttons.find((button) =>
    keys.has(button.getAttribute("data-tab-title"))
  );

  if (firstMatch) activate(firstMatch.getAttribute("data-tab-title"));
}
...
function initFunction() {
  ...
  initSocialShare();
  initAutoFilter();
  updateFilterDetailsMode();
  layoutAllFilterScroll();
  initDisplayReadTime();
  testimonialSlider();
  teamSlider();
...
window.addEventListener("load", () => {
  updateTocLines();
  // Row heights settle once webfonts land, so measure the sidebar again.
  layoutAllFilterScroll();
});
...
(function () {
  function applyHubSpotStyles() {
    const banner = document.getElementById("hs-form-virality-container");

    if (banner) {
      banner.remove();
    }

    document.querySelectorAll(".hs-form input").forEach(function (input) {
      input.classList.add("form_main_field");
    });

    document.querySelectorAll(".hs-form textarea").forEach(function (textarea) {
      textarea.classList.add("form_main_field", "is-textrich");
    });
  }
