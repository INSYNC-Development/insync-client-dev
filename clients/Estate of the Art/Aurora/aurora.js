...
export class Aurora {
  constructor(container, options = {}) {
    ...
    this.resizeObserver.observe(this.ctn);
  }

  init() {
    const geometry = new Triangle(this.gl);
    // Hapus atribut UV jika ada karena tidak dipakai di shader
  ...
  update(t) {
    ...
    // Optional: Jika colorStops sering berubah, update di sini.
    // Jika statis, bisa dipindah ke method terpisah untuk performa.
    // this.program.uniforms.uColorStops.value = ...

    this.renderer.render({ scene: this.mesh });
  }

  // Method untuk update properti secara dinamis
  setOptions(newOptions) {
    this.options = { ...this.options, ...newOptions };
    if (newOptions.colorStops) {
      this.program.uniforms.uColorStops.value = this.options.colorStops.map(
        (hex) => {
          const c = new Color(hex);
          return [c.r, c.g, c.b];
        }
